...
export default function PlantaMedicinal({ size = 160 }: { size?: number }) {
  return (
    <svg className={styles.plantaMedicinal} width={size} height={size} viewBox="0 0 160 200" fill="none" aria-hidden>
      ...
      {[0,72,144,216,288].map((deg, i) => {
        ...
        return <ellipse key={i} cx={80 + 8 * Math.cos(rad)} cy={20 + 8 * Math.sin(rad)} rx="3.5" ry="2.5" transform={`rotate(${deg},${80 + 8 * Math.cos(rad)},${20 + 8 * Math.sin(rad)})`} stroke="#d4a853" strokeWidth="0.7" fill="rgba(212,168,83,0.2)"/>;
      })}
